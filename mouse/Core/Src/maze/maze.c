...
#define M5_VEC_WEST \
  { -1, 0 }
const m5Index M5_DIRECTIONS[4] = {M5_VEC_NORTH, M5_VEC_EAST, M5_VEC_SOUTH,
                                  M5_VEC_WEST};
...
void m5maze_load_from_array(
    m5Maze maze, const char ascii_array[M5_MAZE_SIZE + 1][M5_MAZE_SIZE + 1]) {
...
void m5maze_print_wall(m5Maze maze,
                       uint8_t values[M5_MAZE_SIZE][M5_MAZE_SIZE]) {
  uint8_t has_value = 0;
  if (values) {
    has_value = 1;
  }
...
void m5maze_set_wall(m5Maze maze, m5Index index, const m5Cell *newState) {
  uint8_t y = index.y;
  uint8_t x = index.x;
...
m5IdxQueue m5idxqueue_constructor() {
  m5IdxQueue queue = malloc(sizeof(m5IdxQueueRecord));
  queue->size = M5_MAZE_SIZE * M5_MAZE_SIZE;
  queue->idx_first = queue->idx_last = 0;
  return queue;
}
void m5idxqueue_destructor(m5IdxQueue queue) { free(queue); }

void m5idxqueue_clear(m5IdxQueue queue) { queue->idx_first = queue->idx_last; }
...
void m5maze_update_step_map(m5Maze maze, m5Index destination) {
  ...
  m5IdxQueue queue = m5idxqueue_constructor();
  m5idxqueue_enqueue(queue, destination);

  while (!m5idxqueue_is_empty(queue)) {
    m5Index current_index = m5idxqueue_dequeue(queue);
    ...
    for (uint8_t i = 0; i < 4; i++) {
      ...
      if (!m5cell_is_wall(current_wall, i) &&
        ...
        maze->step_map[scan_index.y][scan_index.x] = current_step + 1;
        if (m5Cell_num_walls(maze->wall[scan_index.y][scan_index.x]) != 3) {
          m5idxqueue_enqueue(queue, scan_index);
        }
      }
    }
  }
  m5idxqueue_destructor(queue);
}
