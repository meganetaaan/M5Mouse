...
export default class Mouse {
  ...
  private readByte(register: number) {
    this.i2c.write(register)
    this.i2c.read(1, this.buf)
    return this.buf[0]
  }
  private readWord(register: number, endian: boolean = true) {
    const buf = this.buf
    this.i2c.write(register)
    this.i2c.read(2, buf)
  ...
  private readBlock(register: number, count: number) {
    this.i2c.write(register)
    return this.i2c.read(count, this.buf)
  }
  private writeByte(register: number, value: any): void {
    return this.i2c.write(register, value & 255)
  }
  private writeWord(register: number, value: any, endian: boolean = true) {
    if (endian) {
